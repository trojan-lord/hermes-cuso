...
**XTTS-v2** (Coqui, ~467M params): Zero-shot voice cloning engine — strong clone fidelity from a **6 s reference**, 17 languages, cross-language cloning, native streaming (<200 ms first chunk). Fits the 4 GB GTX 1650 Ti in fp16 (~2–3 GB VRAM). **⚠️ Weights are CPML-licensed = NON-COMMERCIAL ONLY** (code is MPL-2.0) — disqualifies it for any revenue-generating character voice. Coqui the company shut down Jan 2024; maintained fork `idiap/coqui-ai-TTS` is active (2026); `pip install coqui-tts` still works (0.27.5). CPU-only is slow (~0.2–0.5× RT vs Qwen's ~1.4×). No emotion/instruct control (style comes only from reference clip). Full verified fact sheet, source URLs, and the reusable model-verification method: `references/xtts-v2-fact-sheet.md`.

**F5-TTS** (SWivid, 336M DiT): Flow-matching, fully non-autoregressive — native prosody, auto sentence-chunking + cross-fade (no fixed-silence hack). Fits 4 GB fp16 (~1.2–2 GB VRAM). **⚠️ Code MIT but weights CC-BY-NC-4.0 (non-commercial).** Very active (15k stars, v1.1.22 Jul 2026). `pip install f5-tts`, lightest setup of the three. CPU-only marginal (~0.4–1.5× RT). No emotion tags. Verified Aug 2026 (repo + HF + arXiv 2410.06885).

**CosyVoice2** (Alibaba, now QwenAudio, ~0.64B total): **Only one of the three that's Apache-2.0 for code AND weights (commercial-safe).** Best verified clone similarity (SS 65.9 en vs F5-TTS 64.7, beats it on both zh and en). Native prosody + **instruct mode**: emotion/rate/dialect control, `[laughter]`, `<strong>` emphasis tags — the emotion control the user wants, without fine-tuning. Slowest of the three (T4 fp16 RTF ~1.17; CPU-only effectively hangs — needs GPU). Fits 4 GB fp16 but tight (~2–3 GB, unconfirmed in practice). Repo active (May 2026). Verified Aug 2026 (repo + HF + arXiv 2412.10117).

**RULE — model comparisons need same-text head-to-head (user-caught, Aug 2026):** Paper benchmarks (SIM-o, speaker-similarity scores) compare models against EACH OTHER and against human ground truth — they do NOT prove any model clones better than the user's current Qwen ICL setup, which wasn't in those benchmarks. Do not claim an alternative has "better clone fidelity" without a head-to-head test: same two lines of text through both engines, user listens, user decides. Where the alternatives genuinely win: native prosody (no silence-insertion pipeline), CosyVoice2 instruct-mode emotion control, and license cleanliness — NOT proven clone fidelity.
