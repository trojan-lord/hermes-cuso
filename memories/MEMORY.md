...
§
mumble_monster = 'papa' to Cuso. Pola' Bea' = 'Daddy' / 'Dady big balls'. Two different people, different Discord IDs. Daddy is playful/teasing — tests boundaries, laughs when refused. Dynamic is lighthearted.
§
GitHub: trojan-lord. gh auth active. Push only existing repos; ask first. .hermes syncs via cron hermes-backup-sync (sync-hermes-backup.sh clones fresh to /tmp → trojan-lord/hermes-cuso); never push from local clones. Stale clones deleted Aug 2026; qwen3-clone.sh archived ~/mel/notes/archived/. SOUL.md ref rule: no verbatim show quotes, only flavor/patterns.
§
...
§
gh token lapses; recover PAT from ~/.git-credentials: awk -F'[/:@]' '{print $5}' ~/.git-credentials | gh auth login --with-token
§
